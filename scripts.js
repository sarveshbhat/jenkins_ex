...
function updateGrid() {
    ...
    // Apply the grid scaling (only scale the cells, not the grid container itself)
    const cells = grid.querySelectorAll(".cell");
    cells.forEach(cell => {
        cell.style.transform = `scale(${gridScale})`;
        cell.style.transformOrigin = "top left"; // Ensures the scale happens from the top-left corner
    });

    // Reapply the transformation for any draggable elements (e.g., textboxes)
    const allShapes = document.querySelectorAll(".draggable");
    allShapes.forEach(shape => {
        const rect = shape.getBoundingClientRect();
        // Recalculate the position of the element relative to the grid container
        shape.style.left = `${rect.left}px`;
        shape.style.top = `${rect.top}px`;

        shape.style.transform = `scale(${gridScale})`; // Apply the scaling
        shape.style.transformOrigin = "top left"; // Scale from the top-left corner
    });
}

function zoomGrid(event) {
    gridScale += event.deltaY > 0 ? -0.1 : 0.1;
    gridScale = Math.max(0.1, Math.min(gridScale, 10)); // Limit zoom scale
    updateGrid();
...
function addTextbox() {
    ...
    // Apply scaling after adding the textbox
    textbox.style.transform = `scale(${gridScale})`;
}
